...
## Первый шаг в эфемерной среде (облачная сессия)

Рабочее окружение Claude — память проекта, скиллы, скрипты — живёт в приватном
репозитории `dvsilkin/claude-workspace`, а не здесь: каталог `.claude/` намеренно
не версионируется, чтобы ничего личного не попало в репозиторий проекта.

**Если `~/claude-workspace` отсутствует — подними его перед работой:**

```bash
git clone --depth 1 https://github.com/dvsilkin/claude-workspace.git ~/claude-workspace
~/claude-workspace/scripts/link-workspace.sh "$(pwd)" corehelperlab-site
```

Скрипт подключит память проекта и скиллы симлинками. Он **не затирает** скиллы,
которые среда положила сама: если каталог занят чужим, наши линкуются поштучно.

Локально (мак, Ubuntu) этого делать не нужно — там всё уже подключено.

Оговорки для эфемерной среды:
- клон **мелкий** (`--depth 1`): `git log` и `blame` по памяти работать не будут.
  Понадобится история или запись в память — сначала
  `git -C ~/claude-workspace fetch --unshallow`;
- приватный репозиторий должен быть в GitHub-scope сессии — это разовая
  настройка, но если клон не проходит, причина обычно в ней;
- изменения памяти из эфемерной сессии нужно **закоммитить и запушить**, иначе
  они исчезнут вместе с контейнером.
